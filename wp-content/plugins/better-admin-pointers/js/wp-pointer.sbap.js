...
/**
 * Pointer jQuery widget. Modified by SS to incorporate passed nudge variables.
 */
(function($){
	var identifier = 0,
		zindex = 9999999;
	...
	$.widget('wp.pointer', {
		...
		_processPosition: function( position ) {
			...
			if ( ! result.edge )
				return result;
//correct for missing + and null values
...
if (result.zindexitem) {
		...
		if (result.nudgevertical) {
		var NVstr = result.nudgevertical + barremove;
		var NVfc = NVstr.charAt(0);
		if (NVfc !== '-') {
		NVfc = '+';
		} else {
		NVfc ='';
		}
		NVstr = NVfc + result.nudgevertical + barremove;
		} else {
		NVstr = 0 + barremove;
		}
		
		if (result.nudgehorizontal) {
		var NHstr = result.nudgehorizontal;
		var NHfc = NHstr.charAt(0);
		if (NHfc !== '-') {
		NHfc = '+';
		} else {
		NHfc ='';
		}
		NHstr = NHfc + result.nudgehorizontal;
		} else {
		NHstr = 0;
		}
		
		// Get whether a Horiz or Vert direction
		if ((result.align == 'right') || (result.align == 'left')) {
				aVal = result.align+NHstr;
				} else {
				aVal = result.align+NVstr;
				}
				
		if ((result.edge == 'right') || (result.edge == 'left')) {
				edVal = result.edge+NHstr;
				opEdVal = opposite[result.edge]+NHstr
				} else {
				edVal = result.edge+NVstr;
				opEdVal = opposite[result.edge]+NVstr
				}		
			...
			if ( result.edge == 'top' || result.edge == 'bottom' ) {
				...
				result.at = result.at || aVal + ' ' + opEdVal;
				result.my = result.my || aVal + ' ' + edVal;
//alert("align:" + result.align + " my:" + result.my + " at:" + result.at);				
				
			} else {
				result.align = result.align  || 'top';
//alert("align:" + result.align);
				result.at = result.at || opEdVal + ' ' + aVal;
				result.my = result.my || edVal + ' ' + aVal;
//alert("my:" + result.my + " at:" + result.at);				
			}
