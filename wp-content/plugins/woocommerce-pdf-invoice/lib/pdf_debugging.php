...
<table class="pdfsetup form-table">
  ...
  foreach($defined_constants["user"] as $const => $value) { 
	...
	$find_pdf 	= strpos( $const, 'DOMPDF' );
	$find_debug = strpos( $const, 'DEBUG' );
	$find_wp	= strpos( $const, 'WP_' );
  
  	if ( $find_wp === false && ($find_pdf !== false || $find_debug !== false) ) :
	
  	$rowcount++;
	$row = ($rowcount % 2 == 0 ? 'even' : 'odd');
  ?>
    <tr class="pdf-<?php echo $row; ?>">
      		<th scope="row" class="titledesc title"><?php echo $const; ?></th>
      		<td>
  <?php 
