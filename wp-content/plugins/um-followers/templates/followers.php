<?php if ( $followers ) { ?>
	...
	<?php foreach( $followers as $k => $arr ) { extract( $arr );
	...
	<div class="um-followers-user">
		...
		<div class="um-followers-user-btn">
			...
			if ( $user_id2 == get_current_user_id() ) {
				...
				echo $um_followers->api->follow_button( $user_id2, get_current_user_id() );
			}
			?>
		</div>
		...
		<div class="um-followers-user-name">
			...
			<?php } else if ( $user_id == get_current_user_id() && $um_followers->api->followed( get_current_user_id(), $user_id2 ) ) { ?>
...
<?php } else { ?>
	...
	<div class="um-profile-note"><span><?php echo ( $user_id == get_current_user_id() ) ? __('You do not have any followers yet.','um-followers') : __('This user do not have any followers yet.','um-followers'); ?></span></div>
