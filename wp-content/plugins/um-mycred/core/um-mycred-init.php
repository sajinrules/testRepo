		...
		if ( function_exists( 'mycred_get_users_badges' ) ) :
			...
			if ( ! empty( $badges ) ) {
				...
				foreach ( $badges as $badge_id => $data ) {
					...
					$output .= '<img src="' . get_post_meta( $badge_id, 'main_image', true ) . '" width="'.$size.'" height="'.$size.'" class="mycred-badge earned um-tip-n" alt="' . get_the_title( $badge_id ) . '" title="' . get_the_title( $badge_id ) . '" />';
					$output .= '</span>';
				}
				$output .= '</span>';
			}
