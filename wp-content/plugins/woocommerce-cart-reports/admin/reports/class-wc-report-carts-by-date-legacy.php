...
class WC_Report_Carts_By_Date extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$abandoned_carts = $this->get_count_within_range( $abandoned_carts_items, $this->start_date, $this->chart_interval);

		$legend[] = array(
			'title' => sprintf( __( '%s abandoned carts in this period.', 'woocommerce_cart_reports' ), '<strong>' . sizeof($abandoned_carts) . '</strong>' ),
			'color' => $this->chart_colours['abandoned_carts'],
			'highlight_series' => 1
		);
		$legend[] = array(
			'title' => sprintf( __( '%s orders placed', 'woocommerce_cart_reports' ), '<strong>' . $total_orders . '</strong>' ),
	...
	public function get_count_within_range($data, $start_date, $chart_interval) {
		$results = array();
		foreach($data as $d){
			$date_time = strtotime($d->post_date);
