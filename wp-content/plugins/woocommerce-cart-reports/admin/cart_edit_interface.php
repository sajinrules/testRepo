...
public function woocommerce_cart_items_meta_box($post) {
	...
	<?php if (sizeof($order_items)>0): ?>
		<table cellpadding="0" width="100%" cellspacing="0" class="woocommerce_cart_reports_items">
			<thead >
				<tr>
					<th class="thumb"  width="20%" style="text-align:left;"><?php _e('Item', 'woocommerce_cart_reports'); ?></th>
					...
					<tr class="item <?php echo $table_color; ?>" rel="<?php echo $loop; ?>">
						<td class="thumb" width="20%">
							<a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>" class="tips" data-tip="<?php
								...
								echo '<br/><strong>'.__('Variation ID:', 'woocommerce_cart_reports').'</strong> '; if (isset($item->variation_id)) echo $item->variation_id; else echo '-';
								echo '<br/><strong>'.__('Product SKU:', 'woocommerce_cart_reports').'</strong> '; if (isset($_product->sku)) echo $_product->sku; else echo '-';
						...
						<td class="name" width="20%">
							...
							<div class="row-actions">
								<span class="view"><a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>"><?php _e('View product', 'woocommerce_cart_reports'); ?></a>
							</div>
