...
/*  Copyright 2014  WPBackOffice  (email : support@wpbackoffice.com)
...
if ( is_woocommerce_active() ) {
	...
	*/
	...
	function dashboard_stats_where_abandoned_range( $where ) {
		...
		$where .= " AND post_date > '" . date('Y-m-d G:i:s',  time() +($offset*3600) - ($woocommerce_cart_reports_options['dashboardrange'] * 24 * 60 *60 )) ."' and post_date < '" . date('Y-m-d G:i:s',  time() +($offset*3600)  - $woocommerce_cart_reports_options['timeout']) . "' ";
		return $where;
	}
	...
	function dashboard_stats_where_abandoned_lifetime( $where ) {
		...
		if(WP_DEBUG == true)
			assert(is_numeric($offset));
		$where .= " AND post_date < '" . date('Y-m-d G:i:s',  time() +($offset*3600)  - $woocommerce_cart_reports_options['timeout']) . "' ";
		return $where;
	}
	/*
	...
	function dashboard_stats_where_converted( $where ) {
		...
		if(WP_DEBUG == true)
			assert(is_numeric($offset));
		$where .= " AND post_date > '" . date('Y-m-d G:i:s',  time() + ($offset*3600) - ($woocommerce_cart_reports_options['dashboardrange'] * 60*60 * 24)) . "' ";
		return $where;
	}
