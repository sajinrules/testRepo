...
class ucm_facebook {
	...
	public function init(){
		if(isset($_GET[_SIMPLE_SOCIAL_FACEBOOK_LINK_REWRITE_PREFIX]) && strlen($_GET[_SIMPLE_SOCIAL_FACEBOOK_LINK_REWRITE_PREFIX]) > 0){
			// check hash
			$bits = explode(':',$_GET[_SIMPLE_SOCIAL_FACEBOOK_LINK_REWRITE_PREFIX]);
	...
	public function init_menu(){
		$page = add_submenu_page( 'simple_social_inbox_main', __( 'Facebook Settings', 'simple_social_inbox' ) , __( 'Facebook Settings', 'simple_social_inbox' ) , 'manage_options' , 'simple_social_inbox_facebook_settings' ,  array( $this, 'facebook_settings_page' ) );
	...
	public function page_assets($from_master=false){
		...
		wp_register_style( 'simple-social-facebook-css', plugins_url('networks/facebook/social_facebook.css',_DTBAKER_PLUGIN_FILE_NAME_20_), array(), '1.0.0' );
		wp_enqueue_style( 'simple-social-facebook-css' );
		wp_register_script( 'simple-social-facebook', plugins_url('networks/facebook/social_facebook.js',_DTBAKER_PLUGIN_FILE_NAME_20_), array( 'jquery' ), '1.0.0' );
		wp_enqueue_script( 'simple-social-facebook' );
	...
	public function load_all_messages($search=array(),$order=array()){
		$sql = "SELECT m.*, m.last_active AS `message_time`, mr.read_time FROM `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message` m ";
		$sql .= " LEFT JOIN `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message_read` mr ON m.social_facebook_message_id = mr.social_facebook_message_id";
		$sql .= " WHERE 1 ";
	...
	public function get_message_details($social_message_id){
		...
		foreach($messages as $message){
			...
			$data['social_column_social'] .= '<div><img src="'.plugins_url('networks/facebook/facebook.png', _DTBAKER_PLUGIN_FILE_NAME_20_).'" class="facebook_icon small"><a href="'.$facebook_message->get_link().'" target="_blank">'.htmlspecialchars( $page_or_group ? $page_or_group->get( 'page_name' ) : 'Feed' ) .'</a></div>';
			$data['social_column_summary'] .= '<div><img src="'.plugins_url('networks/facebook/facebook.png', _DTBAKER_PLUGIN_FILE_NAME_20_).'" class="facebook_icon small"><a href="'.$facebook_message->get_link().'" target="_blank">'.htmlspecialchars( $facebook_message->get_summary() ) .'</a></div>';
			// how many link clicks does this one have?
			$sql = "SELECT count(*) AS `link_clicks` FROM ";
			$sql .= " `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message` m ";
			$sql .= " LEFT JOIN `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message_link` ml USING (social_facebook_message_id) ";
			$sql .= " LEFT JOIN `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message_link_click` lc USING (social_facebook_message_link_id) ";
			$sql .= " WHERE 1 ";
			...
			$data['social_column_links'] .= '<div><img src="'.plugins_url('networks/facebook/facebook.png', _DTBAKER_PLUGIN_FILE_NAME_20_).'" class="facebook_icon small">'. $link_clicks  .'</div>';
		}
	...
	public function get_unread_count($search=array()){
		...
		$sql = "SELECT count(*) AS `unread` FROM `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message` m ";
		$sql .= " WHERE 1 ";
		$sql .= " AND m.social_facebook_message_id NOT IN (SELECT mr.social_facebook_message_id FROM `"._SIMPLE_SOCIAL_DB_PREFIX."social_facebook_message_read` mr WHERE mr.user_id = '".(int)get_current_user_id()."' AND mr.social_facebook_message_id = m.social_facebook_message_id)";
			...
			case 'modal':
				if(isset($_REQUEST['socialfacebookmessageid']) && (int)$_REQUEST['socialfacebookmessageid'] > 0) {
					...
					if($ucm_facebook_message->get('social_facebook_message_id') == $_REQUEST['socialfacebookmessageid']){
						...
						include( trailingslashit( $simple_social_inbox_wp->dir ) . 'networks/facebook/facebook_message.php');
					}
