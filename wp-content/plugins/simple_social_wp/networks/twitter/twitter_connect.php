...
?>
	<div class="wrap">
	<h2>
		<?php _e( 'Twitter Connect', 'simple_social_inbox' ); ?>
	</h2>
	<?php
...
function simple_social_twitter_error($msg) {
?>
...
function request_token($tmhOAuth) {
  ...
  if ($code != 200) {
    simple_social_twitter_error("There was an error communicating with Twitter. {$tmhOAuth->response['response']}");
    return;
  }
  ...
  if ($_SESSION['oauth']['oauth_callback_confirmed'] !== 'true') {
    simple_social_twitter_error('The callback was not confirmed by Twitter so we cannot continue.');
  } else {
...
function access_token($tmhOAuth) {
  ...
  if ($params['oauth_token'] !== $_SESSION['oauth']['oauth_token']) {
    simple_social_twitter_error('The oauth token you started with doesn\'t match the one you\'ve been redirected with. do you have multiple tabs open?');
    session_unset();
    return;
  }

  if (!isset($params['oauth_verifier'])) {
    simple_social_twitter_error('The oauth verifier is missing so we cannot continue. did you deny the appliction access?');
    session_unset();
    return;
  }
