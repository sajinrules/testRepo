...
define('_SIMPLE_SOCIAL_TWITTER_LINK_REWRITE_PREFIX','sstlnk');
...
class ucm_twitter {
	...
	public function init(){
		if(isset($_GET[_SIMPLE_SOCIAL_TWITTER_LINK_REWRITE_PREFIX]) && strlen($_GET[_SIMPLE_SOCIAL_TWITTER_LINK_REWRITE_PREFIX]) > 0){
			// check hash
			$bits = explode(':',$_GET[_SIMPLE_SOCIAL_TWITTER_LINK_REWRITE_PREFIX]);
	...
	public function init_menu(){
		...
		$page = add_submenu_page( 'simple_social_inbox_main', __( 'Twitter Settings', 'simple_social_inbox' ) , __( 'Twitter Settings', 'simple_social_inbox' ) , 'manage_options' , 'simple_social_inbox_twitter_settings' ,  array( $this, 'twitter_settings_page' ) );
	...
	public function page_assets($from_master=false){
		...
		wp_register_style( 'simple-social-twitter-css', plugins_url('networks/twitter/social_twitter.css',_DTBAKER_PLUGIN_FILE_NAME_20_), array(), '1.0.0' );
		wp_enqueue_style( 'simple-social-twitter-css' );
		wp_register_script( 'simple-social-twitter', plugins_url('networks/twitter/social_twitter.js',_DTBAKER_PLUGIN_FILE_NAME_20_), array( 'jquery' ), '1.0.0' );
		wp_enqueue_script( 'simple-social-twitter' );
		wp_register_script( 'twitter-text', plugins_url('networks/twitter/twitter-text.js',_DTBAKER_PLUGIN_FILE_NAME_20_), array( 'jquery' ), '1.0.0' );
	...
	public function get($key){
		return get_option('simple_social_twitter_'.$key);
	}
	public function update($key,$value){
		update_option('simple_social_twitter_'.$key, $value);
	}
	...
	public function load_all_messages($search=array(),$order=array()){
		...
		$sql = "SELECT m.*, mr.read_time FROM `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message` m ";
		$sql .= " LEFT OUTER JOIN `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message_read` mr ON m.social_twitter_message_id = mr.social_twitter_message_id";
		$sql .= " WHERE 1 ";
	...
	public function get_message_details($social_message_id){
		...
		foreach($messages as $message){
			...
			$data['social_column_social'] .= '<div><img src="'.plugins_url('networks/twitter/twitter-logo.png', _DTBAKER_PLUGIN_FILE_NAME_20_).'" class="twitter_icon small"><a href="'.$twitter_message->get_link().'" target="_blank">'.htmlspecialchars( $twitter_message->get('twitter_account')->get( 'account_name' ) ) .'</a></div>';
			$data['social_column_summary'] .= '<div><img src="'.plugins_url('networks/twitter/twitter-logo.png', _DTBAKER_PLUGIN_FILE_NAME_20_).'" class="twitter_icon small"><a href="'.$twitter_message->get_link().'" target="_blank">'.htmlspecialchars( $twitter_message->get_summary() ) .'</a></div>';
			// how many link clicks does this one have?
			$sql = "SELECT count(*) AS `link_clicks` FROM ";
			$sql .= " `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message` m ";
			$sql .= " LEFT JOIN `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message_link` ml USING (social_twitter_message_id) ";
			$sql .= " LEFT JOIN `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message_link_click` lc USING (social_twitter_message_link_id) ";
			$sql .= " WHERE 1 ";
			...
			$data['social_column_links'] .= '<div><img src="'.plugins_url('networks/twitter/twitter-logo.png', _DTBAKER_PLUGIN_FILE_NAME_20_).'" class="twitter_icon small">'. $link_clicks  .'</div>';
		}
	...
	public function get_unread_count($search=array()){
		...
		$sql = "SELECT count(*) AS `unread` FROM `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message` m ";
		$sql .= " WHERE 1 ";
		$sql .= " AND m.social_twitter_message_id NOT IN (SELECT mr.social_twitter_message_id FROM `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message_read` mr WHERE mr.user_id = '".(int)get_current_user_id()."' AND mr.social_twitter_message_id = m.social_twitter_message_id)";
	...
	public function output_row($message, $settings){
		...
		<tr class="<?php echo isset($settings['row_class']) ? $settings['row_class'] : '';?> twitter_message_row <?php echo !isset($message['read_time']) || !$message['read_time'] ? ' message_row_unread' : '';?>"
		    ...
		    <td class="social_column_social">
			    <!--<img src="<?php /*echo _BASE_HREF;*/?>includes/plugin_social_twitter/networks/twitter/twitter-logo.png" class="twitter_icon">-->
			    <img src="<?php echo plugins_url('networks/twitter/twitter-logo.png', _DTBAKER_PLUGIN_FILE_NAME_20_);?>" class="twitter_icon">
		    ...
		    <td nowrap class="social_column_action">
		        ...
		        <a href="<?php echo $twitter_message->link_open();?>" class="socialtwitter_message_open social_modal button" data-modaltitle="<?php echo __('Tweet','simple_social_inbox');?>" data-socialtwittermessageid="<?php echo (int)$twitter_message->get('social_twitter_message_id');?>"><?php _e( 'Open' );?></a>
			...
			case 'modal':
				if(isset($_REQUEST['socialtwittermessageid']) && (int)$_REQUEST['socialtwittermessageid'] > 0) {
					...
					if($ucm_twitter_message->get('social_twitter_message_id') == $_REQUEST['socialtwittermessageid']){
						...
						include( trailingslashit( $simple_social_inbox_wp->dir ) . 'networks/twitter/twitter_message.php');
					}
...
class ucm_twitter_message{
	...
	public function mark_as_read(){
		if($this->social_twitter_message_id && get_current_user_id()){
			$sql = "REPLACE INTO `"._SIMPLE_SOCIAL_DB_PREFIX."social_twitter_message_read` SET `social_twitter_message_id` = ".(int)$this->social_twitter_message_id.", `user_id` = ".(int)get_current_user_id().", read_time = ".(int)time();
			ucm_query($sql);
		}
	}
	...
	public function get_summary() {
		...
		if($data && ((isset($data['retweet_count']) && $data['retweet_count'] > 0) || (isset($data['favorite_count']) && $data['favorite_count'] > 0))){
			...
			if(isset($data['retweet_count']) && $data['retweet_count'] > 0){
				$return .= sprintf(__('Retweets: %s', 'simple_social_inbox'),$data['retweet_count']);
			}
			$return .=  ' ';
			if(isset($data['favorite_count']) && $data['favorite_count'] > 0){
				$return .= sprintf(__('Favorites: %s', 'simple_social_inbox'),$data['favorite_count']);
			}
			$return .=  ' )';
		}
		return $return;
	...
	public function output_block($level){
		...
		if($this->get('summary')){
			...
			?>
			<div class="twitter_comment <?php echo $level != 0 ? ' twitter_comment_clickable' : 'twitter_comment_current';?>" data-id="<?php echo $this->social_twitter_message_id;?>" data-link="<?php echo $this->link_open();?>" data-title="<?php echo __('Tweet','simple_social_inbox');?>" data-socialtwittermessageid="<?php echo (int)$this->get('social_twitter_message_id');?>">
				...
				<div class="twitter_comment_body">
					...
					<div class="twitter_comment_stats">
						...
						if($data && ((isset($data['retweet_count']) && $data['retweet_count'] > 0) || (isset($data['favorite_count']) && $data['favorite_count'] > 0))){
							if(isset($data['retweet_count']) && $data['retweet_count'] > 0){
								echo sprintf(__('Retweets: %s', 'simple_social_inbox'),$data['retweet_count']);
							}
							echo ' ';
							if(isset($data['favorite_count']) && $data['favorite_count'] > 0){
								echo sprintf(__('Favorites: %s', 'simple_social_inbox'),$data['favorite_count']);
							}
						} ?>
					</div>
				</div>
