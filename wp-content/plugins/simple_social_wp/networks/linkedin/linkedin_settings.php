...
if($current_account !== false){
	...
	if(isset($_GET['manualrefresh'])){
		...
		<div class="wrap">
			<h2>
				<?php _e( 'LinkedIn Account', 'simple_social_inbox' ); ?>
			</h2>
		<?php
...
//					echo "Saving token as: <br> $access_token <br>";
	                ...
	                ?>
	                <p>Welcome <?php echo htmlspecialchars($user['firstName']);?>! You have successfully connected LinkedIn with the Simple Social plugin. Please click the button below:</p>
			...
			<a href="<?php echo $url; ?>" id="linkedin-login-button"><img
					src="<?php echo plugins_url( 'networks/linkedin/signin-button.png', _DTBAKER_PLUGIN_FILE_NAME_20_ ); ?>"
		...
		<div class="wrap">
			...
			<form action="" method="post">
                ...
                <ul>
                    ...
                    <li>Fill out the details similar to this screenshot (use your own Company Name etc..) <a href="<?php echo plugins_url('networks/linkedin/app-setup.png', _DTBAKER_PLUGIN_FILE_NAME_20_ );?>" target="_blank">click here for screenshot</a> </li>
	                <li>In the OAuth 2.0 Redirect URLs box, put this address: <strong><?php echo admin_url('admin.php?page=simple_social_inbox_linkedin_settings&linkedin_do_oauth_connect');?></strong></li>
				...
				<table class="form-table">
                    ...
                    <tr>
                        <th class="width1">
                            <?php _e( 'API Key', 'simple_social_inbox' ); ?>
                        </th>
                        <td class="">
                    ...
                    <tr>
                        <th class="width1">
                            <?php _e( 'Secret Key', 'simple_social_inbox' ); ?>
                        </th>
                        <td class="">
					...
					<?php if ( $ucm_linkedin_account->get( 'social_linkedin_id' ) ) { ?>
						<tr>
							...
							<td class="">
								<?php echo $ucm_linkedin_account->get( 'last_checked' ) ? ucm_print_date( $ucm_linkedin_account->get( 'last_checked' ), true ) : __( 'N/A', 'simple_social_inbox' ); ?>
							</td>
						</tr>
						<tr>
							<th class="width1">
								<?php _e( 'Network', 'simple_social_inbox' ); ?>
							</th>
							<td class="">
								<div>
						...
						<tr>
							...
							<td class="">
								...
								<strong><?php _e( 'Choose which LinkedIn groups you would like to manage:', 'simple_social_inbox' ); ?></strong><br>
								<?php
								...
								if ( $data && isset( $data['groups'] ) && is_array( $data['groups'] ) && count( $data['groups'] ) > 0 ) {
									...
									_e( 'No linkedin groups Found to Manage', 'simple_social_inbox' );
								}
								?>
							</td>
						</tr>
				...
				<p class="submit">
					<?php if ( $ucm_linkedin_account->get( 'social_linkedin_id' ) ) { ?>
						...
						<input name="butt_save_reconnect" type="submit" class="button"
						       value="<?php echo esc_attr( __( 'Re-Connect to LinkedIn', 'simple_social_inbox' ) ); ?>"/>
						<input name="butt_delete" type="submit" class="button"
						       value="<?php echo esc_attr( __( 'Delete', 'simple_social_inbox' ) ); ?>"
						       onclick="return confirm('<?php _e( 'Really delete this LinkedIn account and all associated data?', 'simple_social_inbox' ); ?>');"/>
					<?php } else { ?>
						<input name="butt_save_reconnect" type="submit" class="button-primary"
						       value="<?php echo esc_attr( __( 'Save and Connect to LinkedIn', 'simple_social_inbox' ) ); ?>"/>
					<?php } ?>
				</p>
	...
	<div class="wrap">
		<h2>
			<?php _e('LinkedIn Accounts','simple_social_inbox');?>
			<a href="?page=<?php echo htmlspecialchars($_GET['page']);?>&social_linkedin_id=new" class="add-new-h2"><?php _e('Add New','simple_social_inbox');?></a>
		</h2>
		<p>
