...
class ucm_linkedin_message{
			...
			foreach($matches[2] as $id => $url){
				...
				if(strlen($url)) {
					...
					if($social_linkedin_message_link_id) {
						...
						$new_link .= _SIMPLE_SOCIAL_LINKEDIN_LINK_REWRITE_PREFIX . '=' . $social_linkedin_message_link_id;
	...
	public function mark_as_read(){
		if($this->social_linkedin_message_id && get_current_user_id()){
			$sql = "REPLACE INTO `"._SIMPLE_SOCIAL_DB_PREFIX."social_linkedin_message_read` SET `social_linkedin_message_id` = ".(int)$this->social_linkedin_message_id.", `user_id` = ".(int)get_current_user_id().", read_time = ".(int)time();
			ucm_query($sql);
		}
	}
...
//		echo '<pre>';print_r($linkedin_data);echo '</pre>';
	...
	public function link_open(){
		return 'admin.php?page=simple_social_inbox_main&social_linkedin_id='.$this->linkedin_account->get('social_linkedin_id').'&social_linkedin_message_id='.$this->social_linkedin_message_id;
	}
