...
$plugin_list=get_transient('esign-auto-downloads');		
	    if($plugin_list)
		{
			 $esig_progress = count($plugin_list)*7000;
		}
...
(function ($) {
	...
	var overlay = $('<div class="page-loader-overlay"></div>').appendTo('body');
        ...
        $(".progress").animate({ width: "100%" }, {
            ...
            start: function () { $(this).before("<div class='load'><p>Updating <a href='https://www.approveme.me/wp-digital-signature-plugin-docs/article/e-signature-wordpress-auto-updates/?progress_bar' target='_blank' style='color:#fff'>WP E-signature</a> Add-ons...</p></div>"); },

            complete: function () { $(this).after("<div class='logo'></div>"); },

            done: function () 
			{ 
			$("div.load").html("<p>Successfully Installed</p>"); 
			$('.page-loader-overlay').hide();
			$(".esig-addon-devbox").hide();
			}
