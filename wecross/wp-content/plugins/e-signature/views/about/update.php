  ...
  <div id="esig-update-popup" style="display:none;">
		...
		<div class="esig-dialog-header">
		  ...
		  <?php _e('we have great news! We recently added some new features along with some 
		   critical security updates for <a href="https://www.approveme.me/wp-digital-e-signature?updates" target="_blank">WP E-Signature</a>. How would you like to handle these updates?','esig'); ?> </p>
		</div>
				...
				if($auto_downloads)
				{
					foreach($plugin_list as $plugin)
					{
						 if(array_key_exists($plugin->addon_id,$auto_downloads))
						   {
						        $link= 'https://www.approveme.me/downloads/' . str_replace( ' ', '-', strtolower( $plugin->item_name ) );
