...
(function($){
				...
				if(esigAjax.esig_mobile == '1')
                {
				...
				else
                 {
					...
					//var recipient_first_name = $('#recipient_first_name');
				     ...
				     tb_show("+ Add signature", '#TB_inline?width=480&height=370&inlineId=signer-signature');
				}
   ...
   if(!sig)
    ...
    if (element.attr('type')=="checkbox"){
	 ...
	 $('.esig-signature-type-add').click(function () {
  ...
  $('body').on('click', '#esig-terms', function () {
      ...
      if(esigAjax.esig_mobile == '1')
       {
         $( "#esig-mobile-popup" ).popup( "close" );  
       }
       
		$.post( esigAjax.ajaxurl+"?action=esig_terms_condition", function( data ) {
		  $('.esig-terms-modal-lg .modal-body').html(data);
