	...
	class Inbound_Auto_Follow_Leads {
		...
		*/
		public static function define_constants() {
			define('INBOUND_TWITTER_CURRENT_VERSION', '1.0.2' );
		...
		*/
		public static function admin_page(){
			...
			$twitter_app_data['ltpc-extra-lead-data-api-key'] = (isset($twitter_app_data['ltpc-extra-lead-data-api-key'])) ? $twitter_app_data['ltpc-extra-lead-data-api-key'] : array();
		...
		*/
		public static function inbound_store_lead_post( $lead_data ) {
			...
			if($lead_data['email'] <> ''){
				...
				$twitter_data =	self::get_full_contact_details( $lead_data['id'] , $lead_data['email']);
						...
						foreach($twitter_app_data['ltpc-extra-lead-data-api-key'] as $key){
							...
							if( $twitter_app_data['ltpc-extra-lead-data-enable-service-'.$counter]){
								...
								if($twitter_app_data['ltpc-extra-lead-data-api-key'] <> '' && $twitter_app_data['ltpc-extra-lead-data-api-secret'] <> '' && $twitter_app_data['ltpc-extra-lead-data-access-token'] <> '' && $twitter_app_data['ltpc-extra-lead-data-access-token-secret'] <> ''){
									...
									self::check_reponse_for_errors_and_save( $lead_data['id'] , $response , $twitter_app_data['ltpc-extra-lead-data-access-token'][$count] );
		...
		*/
		public static function get_full_contact_details( $lead_id , $email_id){
			...
			if( $response_array && $response_array->status == 200){
		...
		*/
		public static function modify_settings_url( $url , $download = null ) {
			if ( !isset($download['zip_filename']) || $download['zip_filename'] != 'inbound-twitter' ) {
				return $url;
			}

			return add_query_arg( array( 'post_type' => 'wp-lead' , 'page'=>'lead-management-twitter-plugin-admin-page' ) , admin_url( 'edit.php' ) );
		}
		...
		*/
		public static function define_pro_settings( $settings ) {
			$settings_url = add_query_arg( array( 'post_type' => 'wp-lead' , 'page'=>'lead-management-twitter-plugin-admin-page' ) , admin_url( 'edit.php' ) );

			$settings['inbound-pro-settings'][] = array(
				'group_name' => INBOUND_TWITTER_SLUG ,
				'keywords' => __('twitter,social' , 'inbound-pro'),
				'fields' => array (
					array(
						'id'  => 'header_twitter',
						'type'  => 'header',
						'default'  => __('Twitter', 'inbound-pro' ),
						'options' => null
					),
					array(
						'id'  => 'header_twitter',
						'type'  => 'html',
						'default'  => sprintf( __('See %s for settings & setup.', 'inbound-pro' ) , '<a href="'.$settings_url.'" target="_blank">'.$settings_url.'</a>' ) ,
					)
				)
