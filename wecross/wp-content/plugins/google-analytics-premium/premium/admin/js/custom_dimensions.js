...
var custom_dimensions = {
	...
	init : function() {
		...
		jQuery(document).on('change', "input[name^=custom_dimensions]", this.row_change_id);
	...
	remove_row : function() {
		...
		var disabled = !!jQuery("#yst_ga-" + new_id + " select:disabled")[0];
	...
	row_change_id : function( ) {
		...
		jQuery("input[name^=custom_dimensions]").each(function(){
		...
		if( total_matches >= 2 ){
			...
			jQuery("#yoast-ga-form-label-table-settings-custom_dimensions").prepend('<tr id="custom-dimension-error"><td colspan="2"><strong><font color="red">' + jQuery("#string_error_custom_dimensions").val() + '</font></strong></td></tr>');
		}
	},
