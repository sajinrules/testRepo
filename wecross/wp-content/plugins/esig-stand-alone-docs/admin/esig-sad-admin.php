...
class ESIG_SAD_Admin {
	...
	public function invite_user_callback(){
		...
		if($doc_id && $email && $url){
			...
			$document = $api->document->getDocument($doc_id); // Get document
			
			if($wpid = get_current_user_id()){
                
				 $owner = $api->user->getUserByWPID(get_current_user_id());
        ...
        if(empty($esig_header_tagline)){
             $esig_header_tagline = __('Sign Legally Binding Documents using a WordPress website','esig-sad')  ; 
        }
        
        $esig_footer_head = apply_filters('esig_invitation_footer_head_filter','');
        if(empty($esig_footer_head)){
             $esig_footer_head = __('What is WP E-Signature?','esig-sad')  ; 
        }
        ...
        if(empty($esig_footer_text)){
            $esig_footer_text = __('WP E-Signature by Approve Me is the
                                ...
                                <b>No monthly fees</b> - <b>Easy to use
                                  WordPress plugin.</b><a style="color:#368bc6;text-decoration:none" href="http://www.approveme.me/wp-digital-e-signature/?ref=1" target="_blank"> Learn more</a> ','esig-sad');
        }
		...
		$template_data = array(
			...
			//$invite_template = file_get_contents(ESIGN_PLUGIN_PATH . DS . 'views' . DS . 'invitations' . DS . 'invite.wtp');
			
			$invite_message = $api->view->renderPartial('invite', $template_data, false, 'invitations');
			
            $subject = "{$document->document_title} - ". __('Signature requested by','esig-sad') . " ".  $sender ;

			$headers = array(
				"From:  ". $sender ." <{$owner->user_email}>",
				"Reply-To: {$owner->first_name} {$owner->last_name} <{$owner->user_email}>"
			);
			
			// Send
			add_filter( 'wp_mail_content_type', array($this, 'mailType') );
			$mailsent = wp_mail($email, $subject, $invite_message, $headers);
			remove_filter( 'wp_mail_content_type', 'set_html_content_type' );

			if(!$mailsent) {
		  $headers  = 'MIME-Version: 1.0' . "\r\n";
		  $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		  $headers .= 'From: '. $sender . '<'. $owner->user_email  .'>' . "\r\n";  
		 $mailsent = wp_mail($email, $subject, $invite_message, $headers); 
		}  
			if($mailsent){
				$result['success'] = true;
	...
	public function document_add_data($data) {
		...
		if(!empty($document_id)){
			...
			$sad = $wpdb->get_row(
				$wpdb->prepare("SELECT * FROM {$this->table} WHERE document_id = %d",
					$document_id
			));
		...
		$select ='<select id="stand_alone_page" class="esig-select2" style="width:200px;" data-placeholder="'.__('Select a page...','esig-sad').'" name="stand_alone_page" data-original="'.$original_val.'">' . "\n" .
			'<option value="none">'.__('Select a page...','esig-sad').'</option>' . "\n";
				
		foreach($pages as $page) :
			$selected = '';
			if(!empty($sad) && $sad->page_id && ($page->ID == $sad->page_id)){
				$selected = __('SELECTED','esig-sad');
			}
			$data_attr = '';
		...
		$html = '
			...
			<div id="stand_alone_options" class="stand_alone_options" style="' . $display_select . '">
				...
				<a href="#" class="tooltip">
					...
					'.__('Display on this page:','esig-sad').'</span> 
				' .$select . '
				<div id="esig-sad-overwrite-modal" style="display:none;">
					<div class="esig-sad-dialog-content">
						'.__('There is already a Stand Alone document on this page. If you do not wish to overwrite it, please choose a different page.','esig-sad').'
					</div>
				</div>
			</div>
			</p>
		...
		$loop_tail .= '
			...
			<div id="esig_sad_popup_hidden" style="display:none;">
				<div class="esig_sad_popup wp-core-ui">
					<p align="center" class="popup-logo"><img src="'. $core_assets_dir .'/images/logo.svg"></p>
					
					<p class="document_title_caption" style="display:none;">
						'.__('dfdf','esig-sad').'
					</p>
					<p class="instructions">
						'.__('Invite someone to sign your document.','esig-sad').'
					</p>
					<form class="invite_form">
						<ul>
							...
							<li>
								<input type="text" name="name" value="" placeholder="James Franco" />
							</li>
							<li>
								<input type="text" name="email" value="" placeholder="james@email.com" />
							</li>
							<li>
								<input class="esig-mini-btn esig-blue-btn" type="submit" name="" value="'. __('Send Invite','esig-sad'). '" />
							</li>
						</ul>
						<div class="loader_wrap">
							<div class="loader" style="display:none;">
								<img src="'. $assets_dir .'/images/loader.gif" />
							</div>
						</div>
					</form>
					<div class="invite_box">
						' .__('Here is the URL for your document.','esig-sad'). '
						<input class="invite_url" name="" value=""/>
						<div class="copy-msg">'.__('Copy instructions go here','esig-sad').'</div>
					</div>
				</div>
			</div>
';
	...
	public function document_index_data($template_data) {
		...
		$count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) " .
			...
			" AND document_status !='%s' AND document_status !='%s'",
			'stand_alone',
			'trash',
			'archive',
			'signed'));
		...
		if(array_key_exists('document_filters', $template_data) ){
			$template_data['document_filters'] .= "| <a href=\"$url\" $css_class >".__('Stand Alone','esig-sad')."</a> ($count)"; 
		}
	...
	public function document_index_docs($docs, $args){
		...
		if('stand_alone' != $document_type) return $docs;
	...
	public function document_after_save($args) {
		...
		$old_page_id = $wpdb->get_var($wpdb->prepare(
			"SELECT page_id FROM {$this->table} WHERE document_id = %d",
			$doc_id
		));
		...
		if(isset($_POST['send_sad']))
				...
				array('document_type'=>'stand_alone','document_status'=> $document_status),
		...
		// Insert/Update
			...
			$wpdb->query(
				$wpdb->prepare("INSERT INTO {$this->table} (document_id, page_id, date_created, date_modified) VALUES(%d, %d, %s, %s) ON DUPLICATE KEY UPDATE page_id = values(page_id), date_modified = values(date_modified)",
				$doc_id,
				$page_id,
				$date_created, 
				$date_created
			...
			if($old_page_id != $page_id){
				//$this->update_shortcode($old_page_id, null);
	...
	private function update_shortcode($page_id, $doc_id){
		...
		$page->post_content = preg_replace('/\[wp_e_signature_sad doc="\d+"\]/', 
			$shortcode, 
			$page->post_content,
			-1,
			$replaced
	...
	public function show_more_actions($more_actions, $args){
        ...
        if (function_exists('has_shortcode'))
        {
		    if(!has_shortcode($page_data->post_content, 'wp_e_signature_sad' ))
		    {
			    ...
			    $api->view->setAlert(array('type'=>'e-sign-red-alert alert e-sign-alert esig-updated', 'title'=>'', 'message'=> sprintf( __( "Oh snap! Your default document page <a href='%1s'>%2s</a> shortcode  has been deleted.", 'esig-sad'), $permalink, $page_title )));
		...
		if($page_id){
			...
			if($doc->document_status != "draft")
						$more_actions .=  '| <a href="edit.php?post_type=esign&page=esign-edit-document&document_id=' . $doc->document_id . '">'.__('Edit','esig-sad').'</a>';
			$more_actions .= ' | <span class="send_stand_alone_invite"><a data-url="' .$url . '" href="javascript:void(0)" data-document="' . $doc->document_id . '" data-title="' . $doc->document_title . '" title="View the Stand Alone Document" id="sad_document_'. $doc->document_id .'">'.__('Share / Invite','esig-sad').'</a></span>';
	...
	public function show_sad_actions($more_option_page, $args){
		...
		$more_option_page .= '<div id="esig-settings-col3">
                ...
                <a href="edit.php?post_type=esign&page=esign-add-document&esig_type=sad" id="sad_view"> 
				<div id="esig-stand-alone" class="esig-doc-options esig-add-document-hover">
					<div class="icon"></div>
					<div class="text">'.__('+ Stand Alone','esig-sad').'</div>
				</div>
                </a>
                <!-- sad document benefits start -->
                <div class="benefits">
					<p>' .  __('Stand Alone Benefits','esig_sad') .'</p>
