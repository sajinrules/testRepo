...
class GravityFormsLeads_Addon extends GFFeedAddOn {
	...
	/**
	*  Adds supportive hooks and filters
	*/
	public function add_hooks() {
		add_filter( 'gform_pre_render', array( $this , 'inject_hidden_field' ) );
		add_filter( 'wp_footer', array( $this , 'populate_hidden_field' ) );
	}
	
	/**
	*  Inject hidden field that contains Inbound Page View object
	*/
	public function inject_hidden_field( $form ) {
		
		$field = new GF_Field_Hidden( array(
			'id' => 9999,
			'type' => 'hidden',
			'label' => 'inbound Page Views',
			'cssClass' => 'inbound_page_views',
			'pageNumber' => 1
		));
	...
	/**
	*  populate hidden field with page_views object from local storage
	*/
	public function populate_hidden_field() {
		?>		
	...
	public function process_feed ( $feed, $entry, $form ) {
		...
		/* Grab pageviews */
		$this->map['page_views'] = (isset($_POST['input_9999'])) ? $_POST['input_9999'] : '';
